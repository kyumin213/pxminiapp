...
Page({
  ...
  getCodeImg: function () {
    wx.request({
      ...
      data: {
        token: this.data.token,
        pagename: 'openBox',
        routes: 'pages/openBox/openBox?ordersId=' + this.data.ordersId
      },
      dataType: 'json',
      header: {
        'content-type': 'application/x-www-form-urlencoded'
  ...
  onShow: function () {
    ...
    wx.getImageInfo({
      src: 'https://image.prise.shop/images/2018/06/07/1528365987727138.png',
      success: (res) => {
        this.setData({
          bgImg: res.path
        })
    ...
    wx.getImageInfo({
      src: 'https://image.prise.shop/images/2018/06/07/1528366013218451.png',
      success: (res) => {
        this.setData({
          bgImgTwo: res.path
        })
  ...
  drawShareImg: function (bgImg, codeImg) {
    ...
    if (this.data.canvasItemList.length > 1) {
      ...
      for (var i = 1; this.getTrueLength(title1) > 0 && i < 3; i++) {
        ...
        ctx.fillText(title1.substr(0, tl).replace(/^\s+|\s+$/, ""), 0.36 * x, 0.91 * x + (i * 20));
        title1 = title1.substr(tl); 
      }

      ctx.drawImage(picture1, 0.17 * x, 0.93 * x, 0.16 * x, 0.16 * x)
      ctx.setFontSize(12)
      ctx.setFillStyle('#999');
      ctx.fillText('共' + num1 + '份', 0.36 * x, 1.08 * x)

      ctx.setFontSize(14);
      ctx.setFillStyle('#333333');
      for (var i = 1; this.getTrueLength(title2) > 0 && i<3; i++) {
        var tl = this.cutString(title2, 24);
        ctx.fillText(title2.substr(0, tl).replace(/^\s+|\s+$/, ""), 0.36 * x, 1.17 * x + (i * 20));
        title2 = title2.substr(tl);
      }

      ctx.drawImage(picture2, 0.17 * x, 1.18 * x, 0.176 * x, 0.176 * x)
      ctx.setFontSize(12)
      ctx.setFillStyle('#999');
      ctx.fillText('共' + num2 + '份', 0.36 * x, 1.33 * x)
    } else {
      var title1 = this.data.canvasItemList[0].productTitle
      var picture1 = this.data.pictureone
      var num1 = this.data.canvasItemList[0].num
      ...
      ctx.setFontSize(12)
      ctx.setFillStyle('#999');
      ctx.fillText('共' + num1 + '份', 0.46 * x, 1.32 * x)
    }
    ctx.setFontSize(12)
  ...
  saveCardPic: function () {
    ...
    // wx.navigateTo({
    //   url: '/pages/openBox/openBox?ordersId=' + this.data.ordersId,
    // })
  },
  /**
   * 用户点击右上角分享
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      title: '开箱子，领礼物！',
      path: '/pages/openBox/openBox?ordersId=' + this.data.ordersId,
      imageUrl: 'https://image.prise.shop/images/2018/06/07/1528365797191882.png',
