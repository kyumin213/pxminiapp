...
Page({
  ...
  getCodeImg: function () {
    wx.request({
      ...
      data: {
        token: this.data.token,
        pagename: 'answerGame',
        routes: 'pages/answerGame/answerGame?ordersId=' + this.data.ordersId
      },
      dataType: 'json',
      header: {
        'content-type': 'application/x-www-form-urlencoded'
  ...
  onShow: function () {
    ...
    wx.getImageInfo({
      src: 'https://image.prise.shop/images/2018/07/05/1530782682162075.png',
      success: (res) => {
        this.setData({
          bgImg: res.path
        })
    ...
    wx.getImageInfo({
      src: 'https://image.prise.shop/images/2018/07/05/1530782677553743.png',
      success: (res) => {
        this.setData({
          bgImgTwo: res.path
        })
    ...
    wx.getImageInfo({
      src: 'https://image.prise.shop/images/2018/07/05/1530785692687882.png',
      success: (res) => {
        this.setData({
          circleImg: res.path
        })
  ...
  drawShareImg: function (bgImg, codeImg) {
    ...
    if (this.data.canvasItemList.length > 1) {
      ...
      for (var i = 1; this.getTrueLength(title1) > 0 && i < 3; i++) {
        ...
        ctx.fillText(title1.substr(0, tl).replace(/^\s+|\s+$/, ""), 0.36 * x, 0.93 * x + (i * 20));
        title1 = title1.substr(tl);
      }

      ctx.drawImage(picture1, 0.17 * x, 0.96 * x, 0.16 * x, 0.16 * x)
      ctx.drawImage(this.data.circleImg, 0.36 * x, 1.08 * x, 0.13 * x, 0.048 * x)
      ctx.setFontSize(12)
      ctx.setFillStyle('#ff2851');
      ctx.fillText('共' + num1 + '份', 0.38 * x, 1.115 * x)
      ...
      for (var i = 1; this.getTrueLength(title2) > 0 && i < 3; i++) {
        ...
        ctx.fillText(title2.substr(0, tl).replace(/^\s+|\s+$/, ""), 0.36 * x, 1.20 * x + (i * 20));
        title2 = title2.substr(tl);
      }

      ctx.drawImage(picture2, 0.17 * x, 1.21 * x, 0.176 * x, 0.176 * x)
      ctx.drawImage(this.data.circleImg, 0.36 * x, 1.34 * x, 0.13 * x, 0.048 * x)      
      ctx.setFontSize(12)
      ctx.setFillStyle('#ff2851');
      ctx.fillText('共' + num2 + '份', 0.38 * x, 1.375 * x)
    } else {
      var title1 = this.data.canvasItemList[0].productTitle
      var picture1 = this.data.pictureone
      var num1 = this.data.canvasItemList[0].num
      ...
      ctx.drawImage(picture1, 0.388 * x, 0.95 * x, 0.24 * x, 0.24 * x)
      ctx.drawImage(this.data.circleImg, 0.44 * x, 1.32 * x, 0.13 * x, 0.048 * x)
      ctx.setFontSize(12)
      ctx.setFillStyle('#ff2851');
      ctx.fillText('共' + num1 + '份', 0.46 * x, 1.355 * x)
    }
    ctx.setFontSize(12)
