...
Page({
  ...
  toggleItem: function () {
    if (this.data.heightstyle == '504rpx') {
      this.setData({
        heightstyle: 220 * this.data.itemList.length + 94 + 'rpx',
        up: true
      })
    } else {
      this.setData({
  ...
  shareHelp: function () {
    wx.navigateTo({
      url: '/pages/assistancePintu/assistancePintu?ordersId=' + this.data.ordersId + '&shareUserId=' + this.data.shareUserId,
    })
  },
  /**
   * 用户点击右上角分享
  ...
  onShareAppMessage: function (res) {
    if (res.from === 'button') {
      ...
      return {
        ...
        path: '/pages/assistancePintu/assistancePintu?ordersId=' + this.data.ordersId + '&shareUserId=' + this.data.shareUserId,
