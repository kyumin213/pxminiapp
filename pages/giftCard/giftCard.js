...
Page({
  ...
  getCodeImg: function () {
    wx.request({
      ...
      data: {
        ...
        routes: 'pages/giftsCardReceiver/giftsCardReceiver?ordersId=' + this.data.ordersId
      },
      dataType: 'json',
      header: {
        'content-type': 'application/x-www-form-urlencoded'
  ...
  onShow: function () {
    ...
    wx.getImageInfo({
      src: 'https://image.prise.shop/images/2018/05/28/1527489700164769.png',
      success: (res) => {
        this.setData({
          bgImg: res.path
        })
    ...
    wx.getImageInfo({
      src: 'https://image.prise.shop/images/2018/05/28/1527489931890020.png',
      success: (res) => {
        this.setData({
          bgImgTwo: res.path
        })
  ...
  drawShareImg: function (bgImg, codeImg) {
    ...
    if (this.data.canvasItemList.length>1) {
      ...
      for (var i = 1; this.getTrueLength(title1) > 0 && i < 3; i++) {
        ...
        ctx.fillText(title1.substr(0, tl).replace(/^\s+|\s+$/, ""), 0.4 * x, 0.65 * x + (i * 20));
        title1 = title1.substr(tl);
      }

      ctx.drawImage(picture1, 0.21 * x, 0.67 * x, 0.16 * x, 0.16 * x)
      ctx.setFontSize(12)
      ctx.setFillStyle('#999');
      ctx.fillText('共' + num1 + '份', 0.4 * x, 0.8*x)
      
      ctx.setFontSize(14);
      ctx.setFillStyle('#333333');
      for (var i = 1; this.getTrueLength(title2) > 0 && i < 3; i++) {
        var tl = this.cutString(title2, 20);
        ctx.fillText(title2.substr(0, tl).replace(/^\s+|\s+$/, ""), 0.4 * x, 0.89 * x + (i * 20));
        title2 = title2.substr(tl);
      }

      ctx.drawImage(picture2, 0.21 * x,0.9 * x, 0.16 * x, 0.16 * x)
      ctx.setFontSize(12)
      ctx.setFillStyle('#999');
      ctx.fillText('共' + num2 + '份', 0.4 * x, 1.04 * x)      
    } else {
      var title1 = this.data.canvasItemList[0].productTitle
      var picture1 = this.data.pictureone
      var num1 = this.data.canvasItemList[0].num
      ...
      ctx.drawImage(picture1, 0.388 * x, 0.65 * x, 0.24 * x, 0.24 * x)
      ctx.setFontSize(12)
      ctx.setFillStyle('#999');
      ctx.fillText('共' + num1 + '份', 0.46 * x, 1.02 * x)
    }
    ctx.setFontSize(12)
