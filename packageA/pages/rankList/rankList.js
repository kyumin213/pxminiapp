...
Page({
  ...
  getCurrentRank: function () {
    utils.httpRequestGet(config.config.getCurrentRank, { token: this.data.token}, (res) => {
      ...
      if(res.data.status == 200) {
        this.setData({
          rankList: res.data.bean.currentList,
          rankRewardTakeWait: res.data.bean.rankRewardTakeWait,
          treeUserId: res.data.bean.recommend
        })
      } else if (res.data.status == 801) {
        utils.getUserInfoFun(this.getCurrentRank, this, this.data.recommend)
      } else {
  ...
  onShareAppMessage: function () {
    return {      
      path: '/packageA/pages/rankList/rankList?treeUserId=' + this.data.treeUserId,
    }
  }
})